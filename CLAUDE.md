...
**API Client (`lib/api-client.ts`)**: Centralized HTTP client with built-in error handling, cookie management, and toast notifications. Handles both server-side and client-side requests with automatic cookie forwarding.

**Environment Management (`config/env.ts`)**: Zod-based environment validation with strict type checking. Currently requires `NEXT_PUBLIC_API_URL`.

**UI Components**: Built on shadcn/ui with Radix UI primitives. Uses Tailwind CSS v4 with CSS variables for theming. Component configuration in `components.json`.
...
### Package Management
Uses pnpm as the package manager with workspace configuration. Lock file is `pnpm-lock.yaml`.
...
- Before creating any new type definitions/interfaces, check the `@/lib/definitions.ts` file
